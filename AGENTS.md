...
3. **所有"总量/剩余"口径 = 体验版基础用量 + 赠送积分**,前端/后端/历史快照/仪表盘必须一致;**基础包剩余必须用 `CycleCapacityRemain`(v1.4.69 修)**:API 体验版包的 `CapacityRemain` 是满额(500)不反映周期消耗,官方 UI"版本基础用量剩余"用 `CycleCapacityRemain`(393.08);赠送包两者恒等,仅基础包需 Cycle* 字段(缺失兜底 Capacity*)
...
18. **同步/测试前端超时(v1.4.49,超时值 v1.4.66 放宽)**:syncAct 按动作放宽 timeout(sync=120s/test=30s)——后端下载/上传各 120s(v1.4.66 由 60s 放宽,防 wb-history.json 慢速穿透超时),默认 15s 会误报超时;新增长耗时接口必须显式传 timeout
...
24. **签到月末溢出 + 同步超时(v1.4.66,镜像膨胀根治 v1.4.67)**:① `detectSignIn` 目标到期日必须「对日+月末钳制」(见第 13 条),否则月末签到漏判(实测 8/31 三账号全部漏报"未签到");② **gc 后必须重导出 `wb-history.json`**——v1.4.66 只在 `fixed>0` 时重导出,但"数据库已固化好、镜像已陈旧"时 `fixed=0` 不触发,9.7MB 镜像(4965 条快照)一直保留;v1.4.67 改为**每次 gc 后无条件 `exportLegacy()`** + **启动时重导出**(`wb-gui.mjs` listen 回调),升级重启即收缩(实测 9.7MB→0.34MB);③ **`importLegacy` 必须跳过已固化旧日**(day_summary 已有 `uin|day` 的快照不导入),否则同步下载把陈旧镜像重新灌入数据库 → 上传重导出又变大,恶性循环;④ gc 失败不再静默吞(console.error + `_gcDay` 失败不置位,当天可重试)
