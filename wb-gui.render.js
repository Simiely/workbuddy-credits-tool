...
function renderCards() {
  ...
  if (!rs.length) {
    ...
    $("foot").textContent = "v1.4.40 · 数据来自 WorkBuddy 网页版接口 · 暂无账号数据(可「添加当前账号」或从 WebDAV 下载)";
    return;
  }
  $("grid").innerHTML = rs.map((r, i) => {
    ...
    // 签到标记（v1.4.40）：由后端 derive 检测今日首条 vs 最新快照的新增满额包推断，见 detectSignIn
  ...
  initDrag();
  $("foot").textContent = "v1.4.40 · 数据来自 WorkBuddy 网页版接口 · 自动刷新 " + autoMin + " 分钟 · 查询失败可重新登录后「添加当前账号」 · 卡片可拖动排序";
}
