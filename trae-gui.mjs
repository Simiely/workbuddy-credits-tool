// trae-gui.mjs - TRAE 积分仪表盘 GUI 服务（薄路由层）
...
async function getDerived() {
  // 读取前先对账:新发现的登录态槽自动入库,新账号即时进派生(与 /api/all/云同步/scan 同一逻辑)
  const accounts = await reconcileFromDisk();
  return deriveAll(accounts);
}

// ---------- 管理员鉴权（默认未启用；首次运行时由前端 /api/admin/setup 设置密码并持久化） ----------
// adminPass 为空 → 开放（本地单用户默认）；设置后写类接口需携带 X-Admin-Token 且匹配才放行。
// 密码持久化在 trae-admin.json（与 wb-sync.json 同级，明文，本地单用户可接受）。
const ADMIN_FILE = path.join(TOOLS_DIR, "trae-admin.json");
let adminPass = "";
try {
...
// admin:true 的路由在分发前统一校验 adminPass（来自 trae-admin.json），handler 内不再重复鉴权。
const routes = [
  // ---------- 静态/传输 ----------
  {
    method: "GET",
  ...
  },
  {
    ...
    path: "/api/accounts",
    handler: async (ctx) => {
      // 与其他读路径一致:读前先对账,新增登录态槽自动入库,账户管理视图与主列表同步可见(单一真相)
      const accounts = (await reconcileFromDisk()).map((a) => ({
        id: a.id,
        name: a.name,
  ...
  },
  {
    ...
    handler: async (ctx) => {
      ...
      const acctSig = accounts.map((a) => a.uin || a.id).join(",");
  ...
  },
  {
    ...
    handler: (ctx) => {
      if (adminPass) return ctx.json(400, { ok: false, error: "管理密码已设置,无法重复设置(如需重置,删除 trae-admin.json 后重启)" });
  ...
  },
  {
    ...
    handler: async (ctx) => {
      ...
      const name = "traework-report-" + new Date().toISOString().slice(0, 10) + ".md";
      ctx.respondRaw(
        200,
  ...
  },
  {
    ...
    handler: (ctx) => {
      ...
      if (history) {
        clearReadings();
        safeRm(path.join(TOOLS_DIR, "trae-history.json"));
        cleared.push("历史快照");
      }
      if (cache) {
        safeRm(LAST_FILE);
  ...
  {
    // 文件导入:兼容两种格式,smart 合并进 SQLite(按 uin 去重取最新),实时刷新前端:
    //   A) trae-accounts.json 账号池快照  {accounts:[...], tombstones:[...]}   (本工具自身导出/WebDAV 镜像)
    //   B) MultiSwitch「🔑导出凭证」json  {token, userId, account:{username}, ...} (v1.5.x,单账号)
    method: "POST",
    path: "/api/import-json",
    admin: true,
    handler: async (ctx) => {
      const body = ctx.bodyObj || {};
      // 先尝试识别为 MultiSwitch 单账号导出:顶层无 accounts 但有 token → 转成账号记录
      const msRec = accountFromMultiSwitchExport(body);
      const incoming = msRec ? [msRec] : Array.isArray(body.accounts) ? body.accounts : [];
      if (!incoming.length)
        return ctx.json(400, { ok: false, error: "文件中无账号数据(既非账号池快照 accounts[], 也非 MultiSwitch 导出凭证)" });
      ...
      ctx.json(200, {
        ...
        ...merged,
        total: local.length,
        _type: msRec ? "multiSwitch-export" : "accounts-snapshot",
        _who: msRec ? (msRec.displayName || msRec.name) : null,
      });
    },
  },
  {
...
function listen(port, max) {
  ...
  server.listen(port, "127.0.0.1", () => {
    ...
    console.log("TRAE 积分仪表盘已启动: " + addr);
