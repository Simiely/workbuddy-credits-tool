...
24. **签到月末溢出 + 同步超时(v1.4.66)**:① `detectSignIn` 目标到期日必须「对日+月末钳制」(见第 13 条),否则月末签到漏判(实测 8/31 三账号全部漏报"未签到");② **gc 清理后必须重导出 `wb-history.json`**(scheduler 的 gc 分支 `fixed>0` 后调 `exportLegacy()`),否则镜像文件陈旧变大(实测 9.7MB/4965 条快照,而 readings 表仅剩 2 天)导致 WebDAV 同步下载/上传超时;③ gc 失败不再静默吞(console.error + `_gcDay` 失败不置位,当天可重试)
