...
// 双模式:
//   独立运行: node edge-daemon.mjs [port=8129] [debugPort=9222]
//   内嵌运行: 被 wb-gui.mjs 调用 createDaemonServer()(SEA 单文件 exe 模式,一个进程同时提供 GUI+daemon)
// API:
...
export function createDaemonServer(opts = {}) {
  ...
  return {
    server,
    connect,
    getStatus: () => ({ connected, port: PORT }),
  };
}

// 独立运行入口(SEA bundle 后 argv[1] 是 exe 路径,不匹配,不会误触发)
const isMain =
  process.argv[1] &&
  (process.argv[1].endsWith("edge-daemon.mjs") ||
    process.argv[1].endsWith("edge-daemon.cjs") ||
    process.argv[1].endsWith("edge-daemon"));
if (isMain) {
  const { server, connect, getStatus } = createDaemonServer();
