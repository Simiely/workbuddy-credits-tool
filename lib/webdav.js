...
/** 删除云端备份文件(DELETE),404/405 视为不存在跳过 */
export async function deleteFile(base, user, pass, dir, file) {
  const r = await req("DELETE", fileUrl(base, dir, file), user, pass);
  if (r.status === 404 || r.status === 405) return false;
  if (r.status >= 200 && r.status < 300) return true;
  throw new Error(`删除 ${file} 失败(HTTP ${r.status})`);
}
