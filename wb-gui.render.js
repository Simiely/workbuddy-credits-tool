...
function renderCards() {
  ...
  if (!rs.length) {
    ...
    $("foot").textContent = "v1.4.47 · 数据来自 WorkBuddy 网页版接口 · 暂无账号数据(可「添加当前账号」或从 WebDAV 下载)";
    return;
  }
  $("grid").innerHTML = rs.map((r, i) => {
    const a = r.account, s = r.summary;
    const nm = escAttr(acctName(a)); // 显示名可自定义,必须转义后进 innerHTML(v1.4.47)
    // 改名/删除:与「今日消耗」同一行,靠右
  ...
  initDrag();
  $("foot").textContent = "v1.4.47 · 数据来自 WorkBuddy 网页版接口 · 页面自动刷新 " + autoMin + " 分钟 · 查询失败可重新登录后「添加当前账号」 · 卡片可拖动排序";
}
