...
- **签到标记**:卡片今日消耗行显示 ✅ 已签到 / ⏰ 未签到(元数据推断,历史签到可回查)
...
- **WebDAV 一键同步**:「🔄 同步」先拉远端合并进本地、再上传合并后全量(双向取最新,无损);删除带墓碑标记跨设备传播(不会因旧备份复活);自动同步定时可选
...
- **签到标记**:每日签到(1-6天100分/天,第7天1000分)通过赠送包元数据推断,账号卡片显示 ✅已签到/⏰未签到 徽标;历史签到固化进日摘要可回查
