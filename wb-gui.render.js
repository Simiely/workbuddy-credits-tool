...
function renderCards() {
  ...
  if (!rs.length) {
    ...
    $("foot").textContent = "v1.4.24 · 数据来自 WorkBuddy 网页版接口 · 暂无账号数据(可「添加当前账号」或从 WebDAV 下载)";
    return;
  }
  ...
  initDrag();
  $("foot").textContent = "v1.4.24 · 数据来自 WorkBuddy 网页版接口 · 自动刷新 " + autoMin + " 分钟 · 查询失败可重新登录后「添加当前账号」 · 卡片可拖动排序";
}
