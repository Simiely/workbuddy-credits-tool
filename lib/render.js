// lib/render.js - 输出渲染层:markdown / CSV / 账号摘要(CLI 与 GUI 共用)
import { displayName } from "./accounts.js";

/** 包名短化(控制台/表格友好) */
export const SHORT_PKG = (n) => (n || "").replace("CodeBuddy个人版国内运营裂变包", "裂变包").replace("CodeBuddy个人体验版", "体验版");

/** 账号对外摘要(不暴露 cookie,供 API/前端展示) */
export function brief(a) {
  return { id: a.id, name: a.name, displayName: a.displayName, uin: a.uin, sessionExpiresAt: a.sessionExpiresAt };
}
...
export function renderSingleMarkdown(D, showExpired, account) {
  ...
  console.log("|---|---|---|---|");
  if (base) console.log(`| 基础用量(${base.PackageName}) | ${base.CapacityUsed}/${base.CapacitySize} | ${base.CapacityRemain} | 当月有效 · 至 ${(base.CycleEndTime || "").slice(0, 10)} |`);
  console.log(`| 权益赠送包(有效) | ${sum(active, "CapacityUsed")}/${sum(active, "CapacitySize")} | ${sum(active, "CapacityRemain")} | ${active.length} 个包 |`);
  console.log(`| 赠送包(已过期) | ${sum(expired, "CapacityUsed")}/${sum(expired, "CapacitySize")} | 0 | ${expired.length} 个包 |`);
  console.log(`| **合计(全部) | ${sum(gifts, "CapacityUsed")}/${sum(gifts, "CapacitySize")} | ${sum(gifts, "CapacityRemain")} | 赠送总计 |`);
  console.log("");
  const rows = showExpired ? gifts : active;
  ...
  rows
    ...
    .forEach((a, i) => {
      console.log(`| ${i + 1} | ${SHORT_PKG(a.PackageName)} | ${a.CapacityUsed}/${a.CapacitySize} | ${a.CapacityRemain} | ${a.CycleEndTime} | ${a.Status === 0 ? "有效" : "已过期"} |`);
    });
}
...
export function renderAllMarkdown(results) {
  ...
  results.forEach((r, i) => {
    if (r.data) {
      const s = r.summary;
      console.log(`| ${i + 1} | ${displayName(r.account)} | ${r.account.uin || "?"} | ${s.baseRemain ?? "-"} | ${s.giftUsed}/${s.giftSize} | ${s.giftRemain} | ${s.giftCount} | ✅ |`);
    } else {
...
export function mdAll(results) {
  ...
  results.forEach((r, i) => {
    ...
    out += `### 总览\n\n| 类别 | 已用/总量 | 剩余 | 说明 |\n|---|---|---|---|\n`;
    if (base) out += `| 基础用量(${base.PackageName}) | ${base.CapacityUsed}/${base.CapacitySize} | ${base.CapacityRemain} | 当月有效 · 至 ${(base.CycleEndTime || "").slice(0, 10)} |\n`;
    ...
    out += `| **合计(全部) | ${sum(gifts, "CapacityUsed")}/${sum(gifts, "CapacitySize")} | ${sum(gifts, "CapacityRemain")} | 赠送总计 |\n\n`;
...
export function csvAll(results) {
  ...
  for (const r of results) {
    if (!r.data) {
      lines.push([displayName(r.account), r.account.uin || "", "查询失败", "-", "-", "-", "-", "-", r.error || ""].join(","));
      continue;
    }
    for (const a of r.data.Accounts) {
      lines.push([displayName(r.account), r.account.uin || "", a.PackageName, a.CapacityUsed, a.CapacitySize, a.CapacityRemain, a.CycleStartTime, a.CycleEndTime, a.Status === 0 ? "有效" : "已过期"].join(","));
    }
  }
